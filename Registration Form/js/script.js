...
const nameRegex = /^[A-Z][a-z]*(\s[A-Za-z]*)*$/s;
if (!name) {
    document.getElementById("nameError").innerHTML = 'Name is required.';
    allOk = false;
} else if (!nameRegex.test(name)) {
    document.getElementById("nameError").innerHTML = 'Name must start with a capital letter and contain only alphabets and spaces.';
    allOk = false;
} else {
 ...
 const usernameRegex = /^[0-9]{2}[A-Za-z]{3}[0-9]{3}$/;
 if (!usernameRegex.test(username)) {
     document.getElementById("usernameError").innerHTML = 'Register number must start with two numbers, followed by three alphabets, and end with three numbers.';
     allOk = false;
 } else {
 ...
 const passwordRegex = /^(?=.*[A-Z])(?=.*[0-9])(?=.*\W)(?=.*[A-z])[a-zA-Z0-9\W]{8,16}$/;
